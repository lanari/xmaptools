function [Si_T,Al_M1,Al_M2,Al_M3,Fe_M1,Fe_M3,Ca_A,Xepi,Xzoi,Xfepi] = StructFctEpidote(Data,handles)
% -
...
%     Fe_M3 Ca_A Xepi Xzoi Xfepi>SiO2 TiO2 Al2O3 FeO MnO MgO CaO Na2O K2O>
%
%   12.5 Oxygens
%
% 
% Created by P. Lanari (Octobre 2013) - Last update 02/10/13.
...
Xepi = zeros(1,length(Data(:,1)));
Xzoi = zeros(1,length(Data(:,1)));
Xfepi = zeros(1,length(Data(:,1)));
...
for i=1:length(Data(:,1)) % one by one
    ...
    if OnCal
        ...
        Xfepi(i) = Fe_M1(i);
        Xepi(i) = Fe_M3(i)-Xfepi(i);
        Xzoi(i) = 1-(Xepi(i)+Xfepi(i));
