function [Aliv,Alvi,Fe3,Fe2,Al_M4,Si_T1,Si_T2,Al_T2,V_M1,Al_M2M3,Al_M1,Mg_M1,Fe_M1,Mg_M2M3,Fe_M2M3,X_MgFe,DeltaLacune,XAme,XCli,XDap,XSud,Xsum] = StructFctChlorite(Data,handles)
% -
...
%
% 
% Created by P. Lanari (July 2010) - Last update 21/08/17.
...
for i=1:length(Data(:,1)) % one by one
    ...
    if OnCal
        ...
        XCli(i) = Mg_M1(i);
        XDap(i) = Fe_M1(i);
        XSud(i) = 1-XAme(i)-XCli(i)-XDap(i); %V_M1(i);
        
        Xsum(i) = XAme(i)+XCli(i)+XDap(i)+XSud(i);
        
        if Aliv(i) < 0 || Alvi(i) < 0 || Al_M4(i) < 0 || XSud(i) < 0 || XDap(i) < 0 || XCli(i) < 0 ||Mg_M2M3(i) < 0 || Fe_M2M3(i) < 0 || Al_M2M3(i) < 0 || Al_T2(i) < 0 || V_M1(i) < 0 || Fe_M1(i) < 0 || Al_M1(i) < 0 || Mg_M1(i) < 0
            Aliv(i) = 0;
            Alvi(i) = 0;
            Fe3(i) = 0;
            Fe2(i) = 0;
