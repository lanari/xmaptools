function [Si4,Aliv,Alvi,Fe2,Fe3,XFe,XMg,Ca_M4,Na_M4,Na_A,V_A,XAl_T2, XSi_T2, XFe3_M1, XFeMg_M1, XAl_M1, XNa_M4, XCa_M4] = StructFctAmphiboles3(Data,handles)
% -
% XMapTools External Function: Structural formula of Ca-amphibole (Fe3+) 
%
%   Note: Fe3+ is estimated using the method proposed by Holland & Blundy (1994)
%
%   [outputs] = function_Name(Data,handles);
%
%   1>Amphibole>CaAmp-StructForm-Fe3>StructFctAmphiboles3>Si4 Aliv Alvi Fe2 
%     Fe3 XFe XMg Ca_M4 Na_M4 Na_A V_A XAl_T2 XSi_T2 XFe3_M2 XFeMg_M2 XAl_M2 
%     XNa_M4 XCa_M4>SiO2 TiO2 Al2O3 FeO MnO MgO CaO Na2O K2O>
%
%   23 Oxygens
%
% 
% Created by P. Lanari (Octobre 2011) - Last update 28/07/12.
...
XFe3_M1 = zeros(1,length(Data(:,1)));
XFeMg_M1 = zeros(1,length(Data(:,1)));
XAl_M1 = zeros(1,length(Data(:,1)));
...
for i=1:length(Data(:,1)) % one by one
    ...
    if OnCal
        ...
        % Structural Formulae (P. Lanari - 2012) Amphiboles Ca
        ...
        FeMg_M1 = 2 - Alvi(i) - Fe3(i) - Ti_norm;  % ajout Ti OK
        
        FeMg_M23 = 3;
        
        FeMg_total = Fe2_norm + Mg_norm;
        
        if (FeMg_total - FeMg_M1 - FeMg_M23 ) > 0
            FeMg_M4 = FeMg_total - FeMg_M1 - FeMg_M23;
        else
            FeMg_M4 = 0;
        end
        ...
        % Atom site repartition : 
        
        XAl_T2(i) = Aliv(i) / 4;
        XSi_T2(i) = 1 - XAl_T2(i);
        XFe3_M1(i) = Fe3(i) / 2;
        XFeMg_M1(i) = FeMg_M1 / 2;
        XAl_M1(i) = Alvi(i) / 2;
        XNa_M4(i) = Na_M4(i) / 2;
        XCa_M4(i) = Ca_M4(i) / 2;
        ...
        if Aliv(i) < 0 || Alvi(i) < 0 || ...
           Ca_M4(i) < 0 || Na_M4(i) < 0 || Na_A(i) < 0 || V_A(i) < 0 || ...
           XAl_T2(i) < 0 || XSi_T2(i) < 0  || XFe3_M1(i) <0 ||  XFeMg_M1(i) <0 || ...
           XAl_M1(i) < 0||  XNa_M4(i)<0 || XCa_M4(i)<0 
...
%         NaA_mes = Na_A(i)
%         disp('* * * ')
%         FeMg_M1_rec = 2*Xed(i) + 2*Xtr(i) + 0*Xts(i) + 1*Xparg(i) + 0*Xmfets(i) + 2*Xcum(i) + 0*Xgl(i)
%         NFeMg_M1_mes = FeMg_M1
...
%         
        %Xtr(i) = FeMg_M1(i)/2 - Xparg(i) - Xed(i) - 2*Xcum(i) ;
