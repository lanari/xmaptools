function [XAlm,XSpe,XPyr,XGro,XAnd,SI,TI,AL,MG,FE2,FE3,MN,CA] = StructFctGarnetFe3(Data,handles)
% -
% XMapTools External Function: Structural formula of Garnet (including Fe3+ estimate)
%
%   [outputs] = function_Name(Data,handles);
%
%   1>Garnet>Gar-StructForm>StructFctGarnet>XAlm XSpe XPyr XGro Si Al Mg Fe
...
%
%
% Created by G. Bonnet (April 2014) - Last update PL 26/04/14.
...
XSpe = zeros(1,length(Data(:,1)));
XPyr = zeros(1,length(Data(:,1)));
XGro = zeros(1,length(Data(:,1)));
XAnd = zeros(1,length(Data(:,1)));
...
for i=1:length(Data(:,1)) % one by one
    ...
    if OnCal
        ...
        XSpe(i) = (1-XAnd(i))*Mn/(Ca+Fe2+Mg+Mn);
        XPyr(i) = (1-XAnd(i))*Mg/(Ca+Fe2+Mg+Mn);
        XAlm(i) = (1-XAnd(i))*Fe2/(Ca+Fe2+Mg+Mn);
        XGro(i) = (1-XAnd(i))*Ca/(Ca+Fe2+Mg+Mn);
        
        %Andradite Si4+(3)Fe3+(2)Ca2+(3)
        %Ferrous   Si4+(3)Al3+(2)XX2+(3)   with XX= Fe2+ Mg2+ Ca2+ or Mn2+
        
        
        if XSpe(i) > 1 | XPyr(i) > 1 | XAlm(i) > 1 | XAnd(i) > 1 | XGro(i) > 1
            SI(i) = 0;
            AL(i) = 0;
            MG(i) = 0;
            FE(i) = 0;
