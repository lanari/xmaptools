function [XAlm,XSpe,XPyr,XGro,SI,TI,AL,MG,FE,MN,CA] = StructFctGarnetTi(Data,handles)
% -
% XMapTools External Function: Structural formula of Garnet (including Ti)
%
%   [outputs] = function_Name(Data,handles);
%
%   1>Garnet>Gar-StructForm-Ti>StructFctGarnetTi>XAlm XSpe XPyr XGro Si Ti 
...
for i=1:length(Data(:,1)) % one by one
    ...
    if OnCal
        ...
        % Solid solution
        XGro(i) = Ca/(Ca+Fe+Mg+Mn);
        XSpe(i) = Mn/(Ca+Fe+Mg+Mn);
        XPyr(i) = Mg/(Ca+Fe+Mg+Mn);
