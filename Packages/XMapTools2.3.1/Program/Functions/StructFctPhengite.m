function [Si4,Al_T2,Si_T2,V_M1,Fe_M1,Mg_M1,Al_M2M3,Mg_M2M3,Fe_M2M3,XMg,XFe,K_A,V_A,Na_A,Sum_A,Xcel,Xmu,Xpyr,Xpar,Xsum] = StructFctPhengite(Data,handles)
% -
...
%
% 
% Created by P. Lanari (July 2010) - Last update 18/08/11.
...
%         XPyr(i) = V_A(i);
%         XPhl(i) = Mg_M1(i); 
%         XMus(i) = Al_T2(i) - XPyr(i); 
%         if Si - 3 - XPyr(i) > 0;
%             XCel(i) = Si - 3 - XPyr(i);
%         else 
%             XCel(i) = 1 - XPyr(i) - XPhl(i) - XMus(i);
%         end
        
%         XPyr(i) = V_A(i);
%         XPhl(i) = Mg_M1(i); % Al_M2M3(i) + Fe + Mn + Mg - 2;
%         XCel(i) = (Fe + Mn + Mg) - 3* XPhl(i); 
%         XMus(i) = (Al_M2M3(i) - (2*XPyr(i) + XCel(i)))/2; 
        
        Xsum(i) = Xpar(i)+Xcel(i)+Xpyr(i)+Xmu(i); 
